...
data_dir = "/CrisisMMD_v2.0/Informative/"
files = {
...
# Disaster categories with numerical labels
disaster_labels = {
    "Non-informative": 0,
    "Earthquake": 1,
    "Flood": 2,
...
disaster_types = list(disaster_labels.keys())[1:]  # Exclude 'Non-informative'

# Function to determine the disaster type
...
def load_and_process_tsv(file_path):
    ...
    # Keep only rows where label_text == label_image
    df = df[df['label_text'] == df['label_image']]

    # Remove duplicate tweet_ids (keeping the first occurrence)
    df = df.drop_duplicates(subset=['tweet_id'])

    # Select relevant columns and rename them
    df = df[['tweet_text', 'label_text', 'event_name']].rename(columns={'tweet_text': 'text', 'label_text': 'label_text'})

    # Convert label_text to numeric values
    df['label'] = df['label_text'].map({'not_informative': 0, 'informative': 1})

    # Assign disaster categories
    df['label_text'] = df.apply(lambda row: get_disaster(row['event_name'], row['label_text']), axis=1)

    # Convert disaster column to numeric values
    df['label'] = df['label_text'].map(disaster_labels)
    df = df[['text', 'label_text', 'label']]
    return Dataset.from_pandas(df, preserve_index=False)

# Create DatasetDict
dataset_dict = DatasetDict({
    split: load_and_process_tsv(data_dir + filename) for split, filename in files.items()
})
