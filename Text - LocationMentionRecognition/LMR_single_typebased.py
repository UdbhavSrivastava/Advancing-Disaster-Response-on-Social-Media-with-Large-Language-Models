...
def read__data(base_path):
    ...
    all_test_sentences = []
    all_test_labels = []
    dataset_name = 'enter the event name here'
    california_folder = os.path.join(base_path, dataset_name)
    
    if os.path.isdir(california_folder):
        train_file = os.path.join(california_folder, 'train.txt')
        dev_file = os.path.join(california_folder, 'dev.txt')
        test_file = os.path.join(california_folder, 'test.txt')

        # Read data from train.txt, dev.txt, and test.txt
        ...
        all_dev_sentences.extend(dev_sentences)
        all_dev_labels.extend(dev_labels)
        all_test_sentences.extend(test_sentences)
        all_test_labels.extend(test_labels)

    return all_train_sentences, all_train_labels, all_dev_sentences, all_dev_labels, all_test_sentences, all_test_labels
...
label2id = {  # Adjust according to your actual labels
    'O': 0,
    'B-CTRY': 1,
    'L-CTRY': 2,
    'U-CITY': 3,
