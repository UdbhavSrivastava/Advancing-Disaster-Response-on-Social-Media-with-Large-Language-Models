...
data_dir = "/scratch/user/udbhav23/CrisisMMD_v2.0/CrisisMMD_v2.0/Humanitarian/"
files = {
...
def load_and_process_tsv(file_path):
    ...
    df = df[df['label_text'].isin(allowed_labels)]

    # Select relevant columns and rename them
    df = df[['tweet_text', 'label_text']].rename(columns={'tweet_text': 'text', 'label_text': 'label_text'})

    # Convert label_text to numeric values
    df['label'] = df['label_text'].map(allowed_labels)
