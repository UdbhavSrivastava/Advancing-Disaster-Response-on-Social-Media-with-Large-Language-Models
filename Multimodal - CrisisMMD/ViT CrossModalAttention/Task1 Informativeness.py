...
class CustomDataCollator:
    ...
    def __call__(self, features):
        ...
        image_features = [x if isinstance(x, torch.Tensor) else torch.tensor(x) for x in image_features]
...
class CrossModalFusion(nn.Module):
    def __init__(self, text_hidden_dim, image_hidden_dim, fusion_dim, num_heads=8):
        super().__init__()
        self.text_proj = nn.Linear(text_hidden_dim, fusion_dim)
        self.image_proj = nn.Linear(image_hidden_dim, fusion_dim)
        self.cross_attn_text = nn.MultiheadAttention(embed_dim=fusion_dim, num_heads=num_heads, batch_first=True)
        self.cross_attn_image = nn.MultiheadAttention(embed_dim=fusion_dim, num_heads=num_heads, batch_first=True)
        self.layer_norm_text = nn.LayerNorm(fusion_dim)
        self.layer_norm_image = nn.LayerNorm(fusion_dim)
    def forward(self, text_tokens, image_tokens):
        text_proj = self.text_proj(text_tokens)
        image_proj = self.image_proj(image_tokens)
        attn_text, _ = self.cross_attn_text(query=text_proj, key=image_proj, value=image_proj)
        text_fused = self.layer_norm_text(text_proj + attn_text)
        attn_image, _ = self.cross_attn_image(query=image_proj, key=text_proj, value=text_proj)
        image_fused = self.layer_norm_image(image_proj + attn_image)
        text_pooled = text_fused.mean(dim=1)
        image_pooled = image_fused.mean(dim=1)
        fused = (text_pooled + image_pooled) / 2
        return fused

class MultiModalClassifier(nn.Module):
    def __init__(self, text_model, image_model, fusion_hidden_dim, num_labels, fusion_dim=1024, num_heads=8):
        super().__init__()
        ...
        self.cross_modal_fusion = CrossModalFusion(text_hidden_dim, image_hidden_dim, fusion_dim, num_heads=num_heads)
        self.classifier = nn.Linear(fusion_dim, num_labels)
    def forward(self, input_ids, attention_mask, pixel_values, labels=None):
        text_outputs = self.text_model.model(input_ids=input_ids, attention_mask=attention_mask, return_dict=True, output_hidden_states=True)
        text_hidden = text_outputs.hidden_states[-1]
        image_outputs = self.image_model(pixel_values=pixel_values, return_dict=True)
        image_hidden = image_outputs.last_hidden_state
        text_hidden = text_hidden.float()
        image_hidden = image_hidden.float()
        fused = self.cross_modal_fusion(text_hidden, image_hidden)
